<?php
    ...
    if (!isset($_SESSION['user'])) {
        header('Location: login.php?Login=Expired');
        exit();
    }
?>
<!DOCTYPE html>
...
<?php
    ...
    if (isset($_POST['name']) && isset($_POST['price']) && isset($_POST['desc'])) {
        ...
        $imagePath = "./image/";
        $target_file = $imagePath.basename($_FILES["image"]["name"]);
        ...
        if (empty($name)) {
            ...
            $sql = 'INSERT INTO `application`(name,price,description,image) VALUES(?,?,?,?)';

            try{

                // Check file exists
                if (file_exists($target_file)) {
                    $error = 'Hình ảnh đã tồn tại. Vui lòng kiểm tra lại.';
                } else {
                    $stmt = $dbCon->prepare($sql);
                    $stmt->execute(array($name,$price,$desc,$image));
